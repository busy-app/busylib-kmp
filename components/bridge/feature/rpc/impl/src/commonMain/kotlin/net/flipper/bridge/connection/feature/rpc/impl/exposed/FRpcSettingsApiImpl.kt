...
import net.flipper.bridge.connection.feature.rpc.impl.model.InternalBrightnessInfo
...
class FRpcSettingsApiImpl(
    ...
    override suspend fun getDisplayBrightness(ignoreCache: Boolean): Result<DisplayBrightnessInfo> {
        return runSuspendCatching(dispatcher) {
            objectCache.getOrElse(ignoreCache) {
                httpClient.get("/api/display/brightness").body<InternalBrightnessInfo>()
            }
        }.map { internalBrightnessInfo -> DisplayBrightnessInfo(internalBrightnessInfo.value) }
    }

    private fun BsbBrightness.asInternalBrightnessInfo(): InternalBrightnessInfo {
        return when (this) {
            BsbBrightness.Auto -> InternalBrightnessInfo("auto")
            is BsbBrightness.Number -> InternalBrightnessInfo("${this.value}")
        }
    }
