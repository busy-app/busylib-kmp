...
class ConnectionSnapshotMergerTest {
    ...
    fun `mergeSnapshots with single snapshot returns it unchanged`() {
        val snapshot = ConnectionSnapshot(
            ...
            status = Disconnected(FInternalDisconnectedReason.OTHER)
        )
    ...
    fun `mergeSnapshots with multiple Disconnected returns Disconnected`() {
        val snapshots = listOf(
            ConnectionSnapshot(status = Disconnected(FInternalDisconnectedReason.OTHER)),
            ConnectionSnapshot(status = Disconnected(FInternalDisconnectedReason.OTHER))
        )

        val result = mergeSnapshots(snapshots)

        assertEquals(Disconnected(FInternalDisconnectedReason.OTHER), result.status)
    }
    ...
    fun `getPriority returns correct ordering`() {
        ...
        val disconnected = getPriority(Disconnected(FInternalDisconnectedReason.OTHER))
