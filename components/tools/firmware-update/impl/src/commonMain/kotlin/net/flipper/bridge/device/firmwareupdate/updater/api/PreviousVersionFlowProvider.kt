            ...
            while (currentCoroutineContext().isActive) {
                fDevicePersistedStorage.getCurrentDeviceFlow()
                    ...
                    .mapLatest { trackedBusyBar ->
                        ...
                        val stateAfterUpdating = fwUpdateFlow
                            .dropWhile { state -> state !is FwUpdateState.Updating }
                            .first { state -> state !is FwUpdateState.Updating }
                        val isUpdatedDeviceCurrent = isDeviceStillCurrent(trackedBusyBar?.uniqueId)
                        if (!isUpdatedDeviceCurrent) return@mapLatest
                        if (stateAfterUpdating !is FwUpdateState.BatteryLow) {
